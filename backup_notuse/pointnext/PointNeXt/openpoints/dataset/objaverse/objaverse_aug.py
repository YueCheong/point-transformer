...
class ObjAugDataSet(Dataset):
    def __init__(self, data_dir, ann_path, args):
        self.data_dir = data_dir
        self.ann_path = '/home/hhfan/code/point-transformer/process/label/test_selected_4_uid_path_idx_dict.json'
        print(f'this is objaugdataset')
        ...
        if self.uniform:
            self.save_path = os.path.join(data_dir, 'objaug%d_%s_%dpts_fps.dat' % (self.num_classes, self.split, self.npoints))
        else:
            self.save_path = os.path.join(data_dir, 'objaug%d_%s_%dpts.dat' % (self.num_classes, self.split, self.npoints))
    ...
    def load_data(self):
        ...
        for uid, info in ann_data.items():
            path = info['path']
            label = info['label']
            pc_path = os.path.join(self.data_dir, path)
...
if __name__ == '__main__':
    ...
    data = ObjAugDataSet(data_dir=data_dir, ann_path=ann_path, args=args)
